...
public class FunctionalInterfaceDemo {
	public static void main(String args[]) {
		...
		List<Student> stuList = Arrays.asList(new Student(1, "Ram"), new Student(2, "Radha"), new Student(3, "Shyam"),
				new Student(4, "Shyam"));

		Comparator<Student> c = (o1, o2) -> o1.getId() - o2.getId();

		List<Student> sortedStuList = stuList.stream().sorted(c).collect(Collectors.toList());
		...
		List<Integer> idList = stuList.stream().collect(Collectors.toMap(Student::getId, Student::getName)).entrySet()
				.stream().map(i -> i.getKey()).collect(Collectors.toList());

		System.out.println("Extract name list From Student Object: " + idList);

		Map<String, List<Student>> groupByName = stuList.stream()
...
/* In Functional interface we can have multiple default and and static method */
@FunctionalInterface
interface MathOpertion {
	int add(int x, int y);
	
	public static void squeare(int x) {
		System.out.println(x);
	}
	
	public static void squeare1(int x) {
		System.out.println(x);
	}
